/**
 * @file odometry.c
 * @brief 里程计与全局定位模块 (Odometry & Global Positioning)
 *
 * 本文件实现了麦克纳姆轮底盘的里程计推算和点到点(Point-to-Point)运动规划功能。
 * 作为第二轮开发(Round 2)的核心模块，它替代了旧版基于简单速度的开环控制。
 *
 * 核心功能：
 * 1. 姿态推算 (Odometry Update):
 *    - 采集四个车轮的编码器反馈。
 *    - 运用正运动学计算底盘(Body Frame)的线速度(vx, vy)。
 *    - 结合 IMU 偏航角(Yaw)，将底盘速度旋转至全局坐标系(Global Frame)。
 *    - 对全局速度进行积分，实时更新小车的绝对坐标 (x_mm, y_mm)。
 *
 * 2. 异常处理 (Anti-Slip Heuristic):
 *    - 内置轻量级的防滑权重计算(Slip Weight)。当轮速期望与反馈差异过大时，
 *      降低该周期的积分权重，防止车轮打滑导致定位彻底跑飞。
 *
 * 3. 点到点运动规划 (Point-to-Point Move):
 *    - 采用分离的坐标轴控制策略(Dominant-axis strategy)，避免到达目标点附近的抖动。
 *    - 支持不同的运动配置(Profile)，如自由行走(WALK)和推箱子(PUSH)，
 *      以适应不同任务对速度和扭矩的需求。
 *
 * 坐标系约定：
 * - 偏航角(Yaw): 顺时针为正 (Clockwise-positive)
 * - 底盘坐标系(Body Frame): x轴向右为正，y轴向前为正
 */

#include "odometry.h"
#include "encoder.h"
#include "low_speed_motion_math.h"
#include "speed_pid.h"
...
static int64_t g_odometry_x_um = 0;
static int64_t g_odometry_y_um = 0;
static int32_t g_point_move_finish_tol_mm = POINT_MOVE_FINISH_TOL_MM;
static int32_t g_slip_weight_filtered_pct = 100;
 ...
 */
static int32_t odometry_calc_slip_weight_raw_pct(void)
{
    int32_t total_error = 0;

    for(int i = 0; i < MOTOR_MAX; i++)
    {
        total_error += abs(speed_pid_get_target_ramped((MotorID)i) -
                           get_speed_now((MotorID)i));
    }
    ...
    {
        int32_t error_span = ODOM_TRACK_ERROR_HARD_PULSE - ODOM_TRACK_ERROR_SOFT_PULSE;
...
static int32_t odometry_filter_slip_weight_pct(int32_t raw_weight_pct)
{
    raw_weight_pct = LIMIT(raw_weight_pct, 0, 100);

    if(raw_weight_pct < g_slip_weight_filtered_pct)
    {
        g_slip_weight_filtered_pct = raw_weight_pct;
    }
    else if(raw_weight_pct > g_slip_weight_filtered_pct)
    {
        g_slip_weight_filtered_pct += ODOM_SLIP_WEIGHT_RECOVER_STEP_PCT;

        if(g_slip_weight_filtered_pct > raw_weight_pct)
        {
            g_slip_weight_filtered_pct = raw_weight_pct;
        }
    }

    return g_slip_weight_filtered_pct;
}
...
static void odometry_integrate_pose_from_raw_delta(int32_t yaw_cd)
{
    ...
    vy_global_um = (int32_t)(-(float)vx_body_um * sin_yaw +
                             (float)vy_body_um * cos_yaw);

    vx_global_um = (int32_t)((float)vx_global_um * ODOM_SCALE_X);
    vy_global_um = (int32_t)((float)vy_global_um * ODOM_SCALE_Y);

    if(g_odometry.slip_weight_pct != 100)
    {
        vx_global_um = (vx_global_um * g_odometry.slip_weight_pct) / 100;
        vy_global_um = (vy_global_um * g_odometry.slip_weight_pct) / 100;
    }

    g_odometry_x_um += vx_global_um;
    g_odometry_y_um += vy_global_um;
    g_odometry.x_mm = (int32_t)(g_odometry_x_um / 1000);
    g_odometry.y_mm = (int32_t)(g_odometry_y_um / 1000);
}
...
void odometry_reset_pose(int32_t start_x_mm, int32_t start_y_mm)
{
    ...
    g_odometry_x_um = (int64_t)start_x_mm * 1000;
    g_odometry_y_um = (int64_t)start_y_mm * 1000;
    g_slip_weight_filtered_pct = 100;
...
void odometry_update(uint32_t period_ms)
{
    ...
    g_odometry.slip_weight_pct =
        odometry_filter_slip_weight_pct(odometry_calc_slip_weight_raw_pct());
    ...
    odometry_integrate_pose_from_raw_delta(yaw_cd);
...
void odometry_set_finish_tolerance_mm(int32_t tolerance_mm)
{
    ...
    if(tolerance_mm < POINT_MOVE_AXIS_STOP_TOL_MM)
    {
        tolerance_mm = POINT_MOVE_AXIS_STOP_TOL_MM;
    }

    g_point_move_finish_tol_mm = tolerance_mm;
}

int32_t odometry_get_finish_tolerance_mm(void)
{
    return g_point_move_finish_tol_mm;
}
                                             ...
                                             int32_t *vy_cmd_mmps)
{
     ...
     */
    if(abs(dx) <= g_point_move_finish_tol_mm &&
       abs(dy) <= g_point_move_finish_tol_mm)
    {
    ...
    if(abs(dx) > abs(dy))
    {
        ...
        if(abs(dy) <= POINT_MOVE_CROSS_AXIS_TOL_MM &&
           abs(dx) > g_point_move_finish_tol_mm)
        {
