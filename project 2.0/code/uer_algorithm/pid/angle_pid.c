...
static int32_t angle_pid_scale_step_limit(int32_t base_step, uint32_t period_ms)
{
    int32_t scaled_step;

    if(base_step <= 0 || period_ms == 0U)
    {
        return 0;
    }

    scaled_step =
        (int32_t)(((int64_t)base_step * (int64_t)period_ms +
                  (EXEC_CONTROL_PERIOD_MS / 2)) /
                 EXEC_CONTROL_PERIOD_MS);

    return (scaled_step > 0) ? scaled_step : 1;
}
...
/*
 * Heading-hold outer-loop implementation.
 *
                              ...
                              uint32_t period_ms)
{
    ...
    /*
     * The heading-hold loop intentionally uses proportional correction plus
     * measured yaw-rate damping, without integral action.
     *
     * A PI-style heading loop can make the chassis shake during lateral
     * translation. That is a common failure mode when integral action is added
     * too early in heading-hold control:
     ...
     *
     * The active heading-hold behavior is therefore:
    ...
    output_step_limit =
        angle_pid_scale_step_limit(heading_hold_output_step_limit, period_ms);
    ...
    if(output_step > output_step_limit)
    {
        raw_output = g_angle.last_output + output_step_limit;
    }
    else if(output_step < -output_step_limit)
    {
        raw_output = g_angle.last_output - output_step_limit;
    }
...
/*
 * Compatibility wrapper for older fixed-period angle-loop call sites.
 */
...
/*
 * Compatibility inner rotation-rate PID for the older direct-rotation path.
 *
 * The active motion chain uses angle_pid_calc_output() to produce a Vz command,
 * then routes that through mecanum inverse kinematics and the four wheel speed
 * loops. This function is kept so old rotation-only code can still link.
 */
...
/*
 * Compatibility PWM distributor for the older direct-rotation path.
 *
 * The active motion chain should prefer Vz -> mecanum inverse kinematics ->
 * wheel speed PID -> motor_set_pwm().
 */
