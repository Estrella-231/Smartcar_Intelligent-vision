...
static volatile int16_t encoder_position_delta[MOTOR_MAX] = {0};
 ...
 * @return void
 */
static EncoderDeltaSample_t encoder_prepare_delta(MotorID motor_id, int16_t raw_count)
{
    return low_speed_encoder_prepare_delta(raw_count,
                                           encoder_forward_sign(motor_id),
                                           SPEED_ZERO_OFFSET);
}
...
void encoder_read_data(void)
{
    ...
    for(int i = 0; i < MOTOR_MAX; i++)
    {
        /* 本次采样的原始脉冲计数值 */
        int16_t raw_count;
        EncoderDeltaSample_t delta_sample;
        /* 电机ID（从数组索引转换） */
         ...
         *      - 保留有效的转速信息
         */
        delta_sample = encoder_prepare_delta(motor_id, raw_count);
        encoder_data[i] = delta_sample.feedback_delta;
        encoder_position_delta[i] = delta_sample.position_delta;
         ...
         */
        change_pos_now(motor_id, get_pos_now(motor_id) + encoder_position_delta[i]);
    }
}
...
{
    return encoder_position_delta[motor_id];
}
