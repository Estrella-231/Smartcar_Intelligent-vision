 ...
 *
 * The caller now provides the real update period, so the integration code must
 * use that runtime value instead of assuming one fixed compile-time cadence.
 *
 * Units:
 ...
 * reuses this field. Keep g_imu.gyro_z_dps2 semantically stable: it always
 * stores the filtered angular acceleration, while the unfiltered derivative
 * stays local to this function.
 */
void IMU_Calc_Rotate_Acc_Period(uint32_t period_ms)
{
    int32_t gyro_acc_raw_cdps2;
    ...
    g_imu.gyro_acc_filtered = (g_imu.gyro_acc_filtered * 4 + gyro_acc_raw_cdps2) / 5;
