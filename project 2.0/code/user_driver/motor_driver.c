 ...
 */
static uint32_t motor_pwm_to_duty(int32_t pwm)
{
    uint32_t pwm_abs = motor_abs_pwm(pwm);

    return (uint32_t)((pwm_abs * PWM_DUTY_MAX + (PWM_MAX_VALUE / 2U)) / PWM_MAX_VALUE);
}
...
void motor_set_pwm(MotorID motor_id, int32_t pwm)
{
    ...
    pwm_set_duty(g_motor[motor_id].hw.pwm_pin, motor_pwm_to_duty(limited_pwm));
